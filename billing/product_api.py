...
from .models import (ApiKey, Audit, Campaign, Contact, CountryPolicy, InboundMessage,
    Message, MessageEvent, Provider, Rate, Route, Sender, Template, Usage, Webhook, WebhookDelivery,
    PhoneNumber,Subaccount,ConsentRecord,BillingAccount)
...
def auth(required="read"):
    def dependency(x_tenant_id:str=Header(...),x_api_key:str|None=Header(None),authorization:str|None=Header(None),db:Session=Depends(session)):
        ...
        aliases={"read":{"read","sms.read"},"messages:write":{"messages:write","sms.send"},"bulk:write":{"bulk:write","sms.bulk"},"webhooks:write":{"webhooks:write","sms.webhook"},"senders:write":{"senders:write","sms.send"},"contacts:write":{"contacts:write","sms.send"},"campaigns:write":{"campaigns:write","sms.bulk"}}
        allowed=aliases.get(required,{required})
        if not scopes.intersection(allowed) and "admin" not in scopes and not (required=="read" and any(s.endswith(":read") for s in scopes)): raise HTTPException(403,"insufficient_scope")
        return x_tenant_id
    return dependency

class ContactIn(BaseModel):
    phone:str=Field(pattern=r"^\+[1-9][0-9]{5,18}$"); name:str|None=None; consent_status:str="unknown"; consent_source:str|None=None; consent_reference:str|None=None; purpose:str|None=None; form_url:str|None=None; terms_version:str|None=None; privacy_version:str|None=None; custom_fields:dict={}
...
class OptOutIn(BaseModel): phone:str=Field(pattern=r"^\+[1-9][0-9]{5,18}$"); scope:str="tenant"; campaign_id:str|None=None; reason:str="api_request"
...
def webhooks(tenant_id:str=Depends(auth()),db:Session=Depends(session)):
    return {"items":[{"id":x.id,"url":x.url,"events":x.events,"enabled":x.enabled,"created_at":x.created_at} for x in db.scalars(select(Webhook).where(Webhook.tenant_id==tenant_id)).all()]}

@router.delete("/webhooks/{webhook_id}",status_code=204)
def delete_webhook(webhook_id:str,tenant_id:str=Depends(auth("webhooks:write")),db:Session=Depends(session)):
    row=db.scalar(select(Webhook).where(Webhook.id==webhook_id,Webhook.tenant_id==tenant_id))
    if not row: raise HTTPException(404,"webhook_not_found")
    db.delete(row);db.add(Audit(tenant_id=tenant_id,actor="api",action="webhook.deleted",target=webhook_id,correlation_id=str(uuid.uuid4())));db.commit()

@router.get("/numbers")
def numbers(tenant_id:str=Depends(auth()),db:Session=Depends(session)):
    rows=db.scalars(select(PhoneNumber).where(PhoneNumber.tenant_id==tenant_id)).all();return {"items":[{"id":x.id,"number":x.number,"account_id":x.account_id,"subaccount_id":x.subaccount_id,"capabilities":x.capabilities,"status":x.status} for x in rows]}

@router.get("/subaccounts")
def subaccounts(tenant_id:str=Depends(auth()),db:Session=Depends(session)):
    rows=db.scalars(select(Subaccount).where(Subaccount.tenant_id==tenant_id)).all();return {"items":[{"id":x.id,"account_id":x.account_id,"name":x.name,"status":x.status} for x in rows]}

@router.post("/opt-outs",status_code=201)
def opt_out(body:OptOutIn,tenant_id:str=Depends(auth("contacts:write")),db:Session=Depends(session)):
    row=db.scalar(select(Contact).where(Contact.tenant_id==tenant_id,Contact.phone==body.phone)) or Contact(tenant_id=tenant_id,phone=body.phone)
    row.consent_status="opted_out";row.opted_out_at=datetime.now(timezone.utc);row.suppression_reason=f"{body.scope}:{body.reason}";db.add(row);db.flush();account=db.scalar(select(BillingAccount).where(BillingAccount.tenant_id==tenant_id));db.add(ConsentRecord(tenant_id=tenant_id,account_id=account.id if account else None,campaign_id=body.campaign_id,phone=body.phone,action="opt_out",source="api",metadata_json={"scope":body.scope,"reason":body.reason}));db.add(Audit(tenant_id=tenant_id,actor="api",action="sms.opted_out",target=row.id,correlation_id=str(uuid.uuid4()),after={"scope":body.scope,"campaign_id":body.campaign_id}));db.commit();return {"id":row.id,"suppressed":True}

@router.get("/opt-outs")
def opt_outs(tenant_id:str=Depends(auth()),db:Session=Depends(session)):
    rows=db.scalars(select(Contact).where(Contact.tenant_id==tenant_id,Contact.opted_out_at!=None)).all();return {"items":[{"id":x.id,"phone":x.phone,"reason":x.suppression_reason,"opted_out_at":x.opted_out_at} for x in rows]}
...
def create_contact(body:ContactIn,request:Request,tenant_id:str=Depends(auth("contacts:write")),db:Session=Depends(session)):
    if body.consent_status=="opted_in" and not body.consent_source: raise HTTPException(422,"consent_source_required")
    values=body.model_dump();form_url=values.pop("form_url");terms_version=values.pop("terms_version");privacy_version=values.pop("privacy_version");row=Contact(tenant_id=tenant_id,**values,consent_at=datetime.now(timezone.utc) if body.consent_status=="opted_in" else None);db.add(row);db.flush()
    if body.consent_status=="opted_in":
        account=db.scalar(select(BillingAccount).where(BillingAccount.tenant_id==tenant_id));ip=(request.client.host if request.client else "unknown");db.add(ConsentRecord(tenant_id=tenant_id,account_id=account.id if account else None,phone=body.phone,action="opt_in",source=body.consent_source or "api",source_ip_hash=hashlib.sha256(ip.encode()).hexdigest(),form_url=form_url,terms_version=terms_version,privacy_version=privacy_version,metadata_json={"reference":body.consent_reference,"purpose":body.purpose}))
...
def webhook(body:WebhookIn,tenant_id:str=Depends(auth("webhooks:write")),db:Session=Depends(session)):
    try:url=validate_webhook_url(str(body.url))
    except ValueError as exc:raise HTTPException(422,str(exc))
    raw="whsec_"+secrets.token_urlsafe(32);row=Webhook(tenant_id=tenant_id,url=url,events=body.events,secret_hash=hashlib.sha256(raw.encode()).hexdigest(),secret_ciphertext=encrypt_secret(raw));db.add(row);db.commit();return {"id":row.id,"secret":raw,"display_once":True}

def queue_webhooks(db,tenant_id,event_id,event_type):
    for hook in db.scalars(select(Webhook).where(Webhook.tenant_id==tenant_id,Webhook.enabled==True)).all():
...
def simulate_mo(body:MOIn,x_simulator_token:str=Header(...),db:Session=Depends(session)):
    ...
    stop=set(policy.stop_keywords if policy else [])|{"STOP","STOPALL","UNSUBSCRIBE","CANCEL","END","QUIT"}; help_words=set(policy.help_keywords if policy else [])|{"HELP"}
    if keyword in stop:
...
def simulate_dlr(body:DLRIn,x_simulator_token:str=Header(...),db:Session=Depends(session)):
    ...
    row=MessageEvent(tenant_id=body.tenant_id,message_id=msg.id,external_event_id=body.external_event_id,type="dlr",status=body.status,provider_response=body.provider_response);msg.status=body.status;db.add(row);db.flush();queue_webhooks(db,body.tenant_id,row.id,"sms."+body.status);db.commit();return {"event_id":row.id,"status":msg.status,"duplicate":False}
