...
def authn(required="read"):
 def dependency(x_tenant_id:str=Header(...),x_api_key:str|None=Header(None),authorization:str|None=Header(None),db:Session=Depends(session)):
    principal=validate_bearer(authorization,x_tenant_id,required)
    if principal:return x_tenant_id
    if not x_api_key:raise HTTPException(401,"authentication_required")
    ...
    scopes=set(row.scopes.split());aliases={"read":{"read","sms.read","billing.read"},"messages:write":{"messages:write","sms.send"},"bulk:write":{"bulk:write","sms.bulk"}}
    if "admin" not in scopes and not scopes.intersection(aliases.get(required,{required})):raise HTTPException(403,"insufficient_scope")
...
@app.get("/healthz")
def healthz(db:Session=Depends(session)):db.execute(select(1));return {"status":"ok"}
@app.get("/ready")
def ready(db:Session=Depends(session)):db.execute(select(1));return {"status":"ready"}
@app.get("/readyz")
def readyz(db:Session=Depends(session)):db.execute(select(1));return {"status":"ready"}
@app.get("/version")
def version():return {"name":"telnexa","version":app.version,"source_sha":os.environ.get("SOURCE_SHA","development"),"simulator":os.environ.get("BILLING_SIMULATOR_ENABLED","true").lower()=="true"}
@app.get("/metrics")
...
def messages(tenant_id:str=Depends(authn()),db:Session=Depends(session)):return {"items":[message_json(x) for x in db.scalars(select(Message).where(Message.tenant_id==tenant_id).order_by(Message.created_at.desc())).all()]}
...
def balance(tenant_id:str=Depends(authn()),db:Session=Depends(session)):return wallet(tenant_id,db)
@app.get("/api/v1/billing/ledger")
def ledger(tenant_id:str=Depends(authn()),db:Session=Depends(session)):return {"items":[{"id":x.id,"amount":str(x.amount),"direction":x.direction,"type":x.type,"reference_id":x.reference_id,"timestamp":x.created_at} for x in db.scalars(select(LedgerEntry).where(LedgerEntry.tenant_id==tenant_id).order_by(LedgerEntry.created_at.desc())).all()]}
@app.get("/api/v1/transactions")
def transactions(tenant_id:str=Depends(authn()),db:Session=Depends(session)):return ledger(tenant_id,db)
@app.get("/api/v1/billing/invoices")
def invoices(tenant_id:str=Depends(authn()),db:Session=Depends(session)):return {"items":[{"id":x.id,"number":x.number,"total":str(x.total),"status":x.status} for x in db.scalars(select(Invoice).where(Invoice.tenant_id==tenant_id)).all()]}
...
def usage_alias(tenant_id:str=Depends(authn()),db:Session=Depends(session)):return usage(tenant_id,db)

@app.post("/api/v1/messages/bulk",status_code=202)
def bulk_send(items:list[SendRequest],idempotency_key:str=Header(...),x_correlation_id:str|None=Header(None),tenant_id:str=Depends(authn("bulk:write")),db:Session=Depends(session)):
    if not items or len(items)>1000:raise HTTPException(422,"bulk_size_must_be_1_to_1000")
    results=[]
    for index,body in enumerate(items):
        results.append(send(body,f"{idempotency_key}:{index}",x_correlation_id or str(uuid.uuid4()),tenant_id,db))
...
def create_key(tenant_id:str,x_admin_token:str=Header(...),db:Session=Depends(session)):
    if not secrets.compare_digest(x_admin_token,os.environ.get('BILLING_ADMIN_TOKEN','disabled')):raise HTTPException(403,"forbidden")
    raw="tnx_"+secrets.token_urlsafe(32);account=db.scalar(select(BillingAccount).where(BillingAccount.tenant_id==tenant_id));row=ApiKey(tenant_id=tenant_id,account_id=account.id if account else None,prefix=raw[:12],secret_hash=ph.hash(raw));db.add(row);db.flush();db.add(Audit(tenant_id=tenant_id,actor="admin",action="api_key.created",target=row.id,correlation_id=str(uuid.uuid4()),after={"prefix":row.prefix,"account_id":row.account_id}));db.commit();return {"id":row.id,"secret":raw,"display_once":True}
...
def shell(title,nav,admin=False):
    links="".join(f"<button data-view='{path}'>{label}</button>" for label,path in (x.split("|",1) for x in nav.split(",")))
