import base64, hashlib, ipaddress, os, socket
from urllib.parse import urlparse
from cryptography.fernet import Fernet, InvalidToken

def _fernet():
    secret=os.environ.get("BILLING_JWT_SECRET","")
    if len(secret)<32: raise RuntimeError("BILLING_JWT_SECRET must contain at least 32 characters")
...
def decrypt_secret(value:str)->str:
    ...
    except InvalidToken as exc:raise RuntimeError("webhook secret decryption failed") from exc
...
def validate_webhook_url(value:str,resolve:bool=True)->str:
    ...
    try: addresses={ipaddress.ip_address(host)}
        ...
        except OSError as exc:raise ValueError("webhook_url_unresolvable") from exc
    if not addresses or any(a.is_private or a.is_loopback or a.is_link_local or a.is_multicast or a.is_reserved or a.is_unspecified for a in addresses):
