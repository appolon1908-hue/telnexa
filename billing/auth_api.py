...
def login(body: Login, request: Request, response: Response, db: Session = Depends(session)):
    ...
    identity = digest(f"{body.tenant_id}:{body.email.lower()}")
    failures = (
        db.scalar(
            select(func.count(LoginAttempt.id)).where(
                LoginAttempt.identity_hash == identity,
                LoginAttempt.ip_hash == ip,
    ...
    member = db.scalar(
        select(TeamMember).where(
            ...
            TeamMember.email == body.email.lower(),
    ...
    try:
        if member:
            ph.verify(member.password_hash, body.password)
            ok = True
    except Exception:
        pass
    ...
    if member.mfa_enabled:
        db.commit()
        raise HTTPException(428, "mfa_required")
    ...
    return {"authenticated": True, "csrf_token": csrf, "role": member.role}


@router.post("/logout", status_code=204)
def logout(request: Request, response: Response, db: Session = Depends(session)):
    raw = request.cookies.get("telnexa_session")
    row = db.scalar(
        select(AuthToken).where(
            AuthToken.token_hash == digest(raw or ""), AuthToken.revoked_at == None
        )
    )
    if row:
    ...
    response.delete_cookie(
        "telnexa_session", secure=True, httponly=True, samesite="strict", path="/"
    )
...
def reset(body: ResetConfirm, db: Session = Depends(session)):
    ...
    member.password_hash = ph.hash(body.new_password)
    row.revoked_at = now
    for s in db.scalars(
        select(AuthToken).where(
