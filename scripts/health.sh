...
for service in rabbitmq redis jasmin webhook-relay nginx prometheus node-exporter billing-db billing-api billing-worker; do
...
"${COMPOSE[@]}" exec -T billing-api python -c "import urllib.request; urllib.request.urlopen('http://127.0.0.1:8000/ready', timeout=4)" || failed=1
